...
import uk.ac.shef.slides.TextSlide;
...
public class SlideGUI extends PFrame
{
		...
		this.getCanvas().addKeyListener(new KeyAdapter(){
			public void keyPressed(KeyEvent e)
			{
				if (e.getKeyCode() == KeyEvent.VK_SPACE)
					...
					prev();
				else if (e.getModifiers() == KeyEvent.ALT_MASK && e.getKeyCode() == KeyEvent.VK_ENTER)
					fullscreen();
	...
	public void selectSlide(double x, double y)
	{
		for (Rectangle2D bounds : titleToBounds.keySet()) {
			...
			{
				this.getCanvas().getCamera().animateViewToCenterBounds(bounds, true, 2000);
	...
	public void setupDisplay()
	{
		...
		for(int i = 0 ; i < slides.size() ; i++)
		{
			...
			node.animateToPositionScaleRotation(location.getX(), location.getY(), 1.0, 0.0, 2000);
			
			titleToBounds.put(new Rectangle2D.Double(location.getX(),location.getY(),node.getWidth(),node.getHeight()),i);
	...
	{
		this.getCanvas().getCamera().animateViewToCenterBounds(new Rectangle2D.Double(0,0,maxX,maxY), true, 2000);
	}
}
